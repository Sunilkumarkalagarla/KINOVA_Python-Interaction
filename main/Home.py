...
def create_angular_action_1(base,actuator_count):
    ...
    for action in action_list.action_list:
        if action.name == "place_2":
            array_new = action.reach_joint_angles.joint_angles.joint_angles._values
    
    for joint_id in range(actuator_count):
        if joint_id+1 == 1:
            joint_angle = action.reach_joint_angles.joint_angles.joint_angles.add()
            joint_angle.value = array_new[0].value
        if joint_id+1== 2:
            joint_angle = action.reach_joint_angles.joint_angles.joint_angles.add()
            joint_angle.value = array_new[1].value
        if joint_id + 1== 3:
            joint_angle = action.reach_joint_angles.joint_angles.joint_angles.add()
            joint_angle.value = array_new[2].value
        if joint_id +1== 4:
            joint_angle = action.reach_joint_angles.joint_angles.joint_angles.add()
            joint_angle.value = array_new[3].value
        if joint_id +1== 5:
            joint_angle = action.reach_joint_angles.joint_angles.joint_angles.add()
            joint_angle.value =array_new[4].value
        if joint_id + 1== 6:
            joint_angle = action.reach_joint_angles.joint_angles.joint_angles.add()
            joint_angle.value = array_new[5].value
...
def example_move_to_home_position(base):
    ...
    # for action in action_list.action_list:
    #     if action.name == "place_2":
    #         print(action.reach_joint_angles.joint_angles.joint_angles._values[1].value)
...
def example_create_sequence(base, base_cyclic):
    ...
    angular_action_1 = create_angular_action_1(base,actuator_count)
    ...
    task_2.action.CopyFrom(angular_action_1)
